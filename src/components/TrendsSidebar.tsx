...
/* //TODO: Uncomment this in production so that it uses the Postgres version then remove the SQLite version below
...
const getTrendingTopics = unstable_cache(
    async () => {
        const result = await prisma.$queryRaw<{ hashtag: string; count: number }[]>`
        SELECT LOWER(SUBSTR(content, INSTR(content, '#'))) AS hashtag, COUNT(*) AS count
        FROM posts
        WHERE content LIKE '%#%'
        GROUP BY hashtag
        ORDER BY count DESC, hashtag ASC
